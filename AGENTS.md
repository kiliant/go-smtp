...
This repository keeps agent policy under both `.claude/` and `.codex/`. Codex
must consult the Claude configuration rather than treating it as configuration
for a different tool that can be ignored:

- Read [`.claude/settings.json`](.claude/settings.json) before running shell,
  Git or container commands. Its `permissions.allow` entries record command
  families the repository owner intends agents to use without an additional
  project-level confirmation; its `permissions.ask` entries always require an
  explicit approval request.
- Claude's permission file does not override Codex's own sandbox, approval or
  safety controls. When Codex still requires runtime approval for an allowed
  command, request it normally. If the two policies differ, follow the stricter
  one; never work around either policy.
- Role instructions in [`.claude/agents/`](.claude/agents/) are the shared
  source for each named project agent. The corresponding
  [`.codex/agents/`](.codex/agents/) profiles must remain semantically aligned.
  A Codex agent using one of those roles reads the applicable Claude role file
  as well as its Codex profile and follows the stricter instruction where they
  differ.

These references make the policies discoverable to Codex; they do not grant
permissions that the active Codex runtime has not granted.
