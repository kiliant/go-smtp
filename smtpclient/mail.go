...
func (c *Client) Mail(ctx context.Context, from string, opts *smtp.MailOptions, send *MailSendOptions) error {
	...
	args, err := c.mailArgs(path, opts, send)
	if err != nil {
		return err
	}
...
func (c *Client) mailArgs(path string, opts *smtp.MailOptions, send *MailSendOptions) ([]string, error) {
	...
	encoded, err := c.encodeParams(params, send.allowUnadvertised())
	if err != nil {
		return nil, err
	}
